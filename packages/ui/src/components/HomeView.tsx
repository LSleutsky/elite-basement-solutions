import { ArrowRight, Check, Library, LucideIcon } from "lucide-react";
import { ReactNode } from "react";
import { Link } from "react-router";

import { getYearsOfExperience } from "@elite/ui/utils";
...
interface HomeViewProps {
  logo: ReactNode;
  eyebrow: string;
  headlines: { subject: string; accent: string }[];
  bannerText: string;
...
const ServiceCard = ({ card }: { card: HomeServiceCard }) => {
  const Icon = card.icon;
  const blur = card.blurAccent === "teal" ? "bg-elite-teal/10" : "bg-elite-olive/10";

  if (card.featured) {
    return (
      <div className="bg-surface group relative overflow-hidden rounded-2xl p-6 md:row-span-2 md:p-8">
        <div
          className={`${blur} absolute -top-12 -right-12 h-32 w-32 rounded-full blur-2xl transition-all group-hover:scale-150`}
        />
        <div className="relative">
          <div className="bg-elite-teal/20 mb-4 inline-flex rounded-full p-3">
            <Icon className="text-elite-olive" />
          </div>
          <h3 className="text-primary mb-2 text-xl font-semibold">{card.title}</h3>
          <p className="text-muted mb-4 text-sm leading-relaxed md:text-base">{card.description}</p>
          {card.features && (
  ...
  return (
    <div
      className={`bg-surface group relative overflow-hidden rounded-2xl p-6${card.className ? ` ${card.className}` : ""}`}
    >
      <div
        className={`${blur} absolute -top-8 -right-8 h-24 w-24 rounded-full blur-2xl transition-all group-hover:scale-150`}
      />
      <div className="relative">
        <div className="bg-elite-teal/20 mb-4 inline-flex rounded-full p-3">
          <Icon className="text-elite-olive" />
        ...
        <h3 className="text-primary mb-2 text-xl font-semibold">{card.title}</h3>
        <p className="text-muted mb-4 leading-relaxed">{card.description}</p>
        <Link
          className="text-elite-teal inline-flex items-center gap-1 text-sm font-medium"
          to={card.href}
        >
        ...
        <div className="bg-surface order-2 rounded-2xl p-6 md:col-span-2 md:p-8 lg:col-span-2 lg:col-start-2 lg:row-span-2">
          ...
          <div className="space-y-3 md:space-y-4">
            {headlines.map((line) => (
              <h1
                key={line.subject}
                className="font-serif text-xl leading-tight sm:text-2xl md:text-4xl lg:text-5xl"
              >
                <span className="text-primary">We make </span>
                <span className="text-primary">{line.subject} </span>
                <span className="text-elite-teal">{line.accent}</span>
              </h1>
            ))}
          </div>
        </div>
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-[#0d0d0d] p-6 md:col-span-2 lg:col-span-2">
          <div className="flex flex-col items-center justify-center gap-4 py-4 text-center md:flex-row md:justify-between md:text-left">
            <div>
              <p className="text-lg font-medium text-white md:text-xl">{customSolutionTitle}</p>
